...
export default function Header() {
  return (
    <header className="flex flex-row justify-between items-center p-2 border-b border-border">
      <div className="flex flex-row items-center gap-2">
        <a href={"https://albertoharka.it/"} target="_blank">
          <Brand className="w-12 h-12 fill-foreground" />
        </a>
        <Link href={"/"} className="cursor-pointer active:shadow-inner active:scale-95">
          <div className="h-12 p-2 border border-border rounded">
            <Logo>
              <p className={cn(saira.className, "text-lg select-none tracking-tight")}>panoptico</p>
            </Logo>
          </div>
        </Link>
      </div>
